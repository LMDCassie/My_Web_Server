...
int func(SOCKET ClientSocket)
{
	...
	if (iResult > 0) {
		...
		if (relativePath.compare("/favicon.ico") == 0)
			absolutePath = "C:/Users/Cassie/Desktop/web_server/html/favicon.ico";
		else
			absolutePath = "C:/Users/Cassie/Desktop/web_server/html/" + relativePath.substr(5);
		...
		if (method.compare("GET") == 0)
		{
			...
			else
			{
				...
				if (fin)
					std::cout << "all characters are read successfully.";
				else
		...
		else if (method.compare("POST") == 0)
		{
			...
			bool isSuccess = true;
			if (body.find("login=3170104768&") == std::string::npos)
				isSuccess = false;
			pos = body.find("=4768");
			if (body.find("pass=4768") == std::string::npos || (pos + 6) <= body.size())
				isSuccess = false;
			sendPacket += "HTTP/1.1 ";
			if (isPOST)
				sendPacket += "200 OK\r\n";
